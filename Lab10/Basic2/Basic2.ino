...
void setup() {
  ...
  lcd.backlight();     //open LCD backlight
   xTaskCreate(LoginTask, (const portCHAR *) "task_login", 128, NULL, 3, &handleLogin);
   xTaskCreate(DisplayTask, (const portCHAR *) "task_display", 128, NULL, 2, &handleDisplay);
    ...
    if (key == '#') {
      ...
      */
      if(firstInput == 0){  //enter setup
        ...
        //vTaskSuspend(handleLogin);
        //vTaskResume(handleSetup);
      }
      if(firstInput == 1){
        if (count == passLength && wrong != 1) {
          ...
          vTaskPrioritySet(handleLogin, 2); 
        }
